...
export default function ArrayView({ ...props }) {
  ...
  const displayKeys = () =>
    Object.keys(props.value[0] || {}).filter(
      (x) => !(props.hiddenKeys || []).includes(x)
    )
        ...
        <TableBody>
          {props.value.map((row, index) => (
            <React.Fragment>
              ...
              {!!props.hasSpanningRowForIndex &&
                !!props.spanningRowForIndex &&
                props.hasSpanningRowForIndex(index) && (
                  <TableRow key={`${index}-optional`}>
                    <TableCell colSpan={displayKeys().length}>
                      {props.spanningRowForIndex(index)}
                    </TableCell>
                  </TableRow>
                )}
            </React.Fragment>
